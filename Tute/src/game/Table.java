...
public class Table {
    ...
    /**
     * Won plays. List of plays (set of cards) which the player has won over the course of the game.
     */
    private LinkedHashMap<Player, ArrayList<ArrayList<Cards>>> tricks = new LinkedHashMap<>();
    ...
    private Cards dealCard(Player player, boolean Triunfo) {
        ...
        if (Triunfo) this.Triunfo = dealingCard;
        deck.remove(0);
        return dealingCard;
    }
    ...
    public void startGame() {
        ...
        for (int i = 1; i<=TOTAL_PLAYS_NUMBER+1; i++) {
            ...
            wonPlay.getKey().addPoints(Cards.calculatePoints(new ArrayList<>(currentPlay.values())));
                ...
                players.forEach( (p) -> {System.out.println(((p instanceof Human)?ANSI_BLUE:"")+ p + "\t - \t" + p.getPoints() +((p instanceof Human)?ANSI_RESET:""));});
                System.out.println();
